...
class DCN(object):
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        ...
        z_center = torch.arange(0, self.depth, device = self.device).repeat([self.width*self.height])
        ...
        y_center = torch.arange(0, self.width, device = self.device).repeat([self.height * self.depth])
        ...
        x_center = torch.arange(0, self.height, device = self.device).repeat([self.depth * self.width])
        ...
        if self.morph == 0:
            z = torch.linspace(0, 0, 1, device=self.device)
            ...
            z = torch.linspace(-int(self.num_points // 2), int(self.num_points // 2), int(self.num_points), device=self.device)
            y = torch.linspace(0, 0, 1, device=self.device)
            x = torch.linspace(0, 0, 1, device=self.device)
            z, y, x = torch.meshgrid(z, y, x)
            z_spread = z.reshape(-1, 1)
    ...
    def _bilinear_interpolate_3D(self, input_feature, z, y, x):
        ...
        value_a0 = torch.index_select(input_feature_flat, dim=0, index=index_a0.type(torch.int64))
        value_b0 = torch.index_select(input_feature_flat, dim=0, index=index_b0.type(torch.int64))
        value_c0 = torch.index_select(input_feature_flat, dim=0, index=index_c0.type(torch.int64))
        value_d0 = torch.index_select(input_feature_flat, dim=0, index=index_d0.type(torch.int64))
        value_a1 = torch.index_select(input_feature_flat, dim=0, index=index_a1.type(torch.int64))
        value_b1 = torch.index_select(input_feature_flat, dim=0, index=index_b1.type(torch.int64))
        value_c1 = torch.index_select(input_feature_flat, dim=0, index=index_c1.type(torch.int64))
        value_d1 = torch.index_select(input_feature_flat, dim=0, index=index_d1.type(torch.int64))
